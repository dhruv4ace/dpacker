...
from .enums import OperationStatus, DhpmLogType
...
def engine_overlay_manager_draw_callback(self, context):
    ...
    try:
        ...
        status_str = self.log_manager.last_log(DhpmLogType.STATUS)
        if status_str is None:
            status_str = ''

        status_color = self.OPSTATUS_TO_COLOR[self.log_manager.operation_status()]
        hint_str = self.log_manager.last_log(DhpmLogType.HINT)
        ...
        log_print_metadata = (\
            (DhpmLogType.INFO,   'INFO'),
            (DhpmLogType.WARNING,'WARNINGS'),
            (DhpmLogType.ERROR,  'ERRORS')
        )
