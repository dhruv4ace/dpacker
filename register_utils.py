...
import subprocess
from .version import DhpmVersionInfo
...
def get_engine_version():
    engine_args = [get_engine_execpath(), '-E', '-o', str(DhpmOpcode.REPORT_VERSION)]
    ...
    while True:
        ...
        if msg_code == DhpmMessageCode.VERSION:
            version_msg = msg
            break

    if version_msg is None:
        raise RuntimeError('Could not read the DHPM version')
...
def parse_feature_codes(feature_codes):
    ...
    for code in feature_codes:
        int_code = code
        if int_code == DhpmFeatureCode.DEMO:
            prefs.FEATURE_demo = True
        elif int_code == DhpmFeatureCode.ISLAND_ROTATION:
            prefs.FEATURE_island_rotation = True
        elif int_code == DhpmFeatureCode.OVERLAP_CHECK:
            prefs.FEATURE_overlap_check = True
        elif int_code == DhpmFeatureCode.PACKING_DEPTH:
            prefs.FEATURE_packing_depth = True
        elif int_code == DhpmFeatureCode.HEURISTIC_SEARCH:
            prefs.FEATURE_heuristic_search = True
        elif int_code == DhpmFeatureCode.PACK_RATIO:
            prefs.FEATURE_pack_ratio = True
        elif int_code == DhpmFeatureCode.PACK_TO_OTHERS:
            prefs.FEATURE_pack_to_others = True
        elif int_code == DhpmFeatureCode.GROUPING:
            prefs.FEATURE_grouping = True
        elif int_code == DhpmFeatureCode.LOCK_OVERLAPPING:
            prefs.FEATURE_lock_overlapping = True
        elif int_code == DhpmFeatureCode.ADVANCED_HEURISTIC:
            prefs.FEATURE_advanced_heuristic = True
        elif int_code == DhpmFeatureCode.SELF_INTERSECT_PROCESSING:
            prefs.FEATURE_self_intersect_processing = True
        elif int_code == DhpmFeatureCode.VALIDATION:
            prefs.FEATURE_validation = True
        elif int_code == DhpmFeatureCode.MULTI_DEVICE_PACK:
            prefs.FEATURE_multi_device_pack = True
        elif int_code == DhpmFeatureCode.TARGET_BOX:
            prefs.FEATURE_target_box = True
        elif int_code == DhpmFeatureCode.ISLAND_ROTATION_STEP:
            prefs.FEATURE_island_rotation_step = True
        elif int_code == DhpmFeatureCode.PACK_TO_TILES:
...
def register_specific(bl_info):
    ...
    addon_engine_path = os.path.join(os.path.dirname(os.path.abspath(os.path.realpath(__file__))), 'dhpm')
    paths_to_check = [get_prefs().engine_path, engine_path, addon_engine_path]
...
def check_engine():
    engine_release_filepath = os.path.join(get_engine_path(), 'release-{}.dhruv'.format(DhpmVersionInfo.engine_version_string()))
...
def register_engine(engine_path):
    ...
    try:
        ...
        if not check_engine():
            raise RuntimeError('Engine version {} required'.format(DhpmVersionInfo.engine_version_string()))
        ...
        if engine_version != DhpmVersionInfo.engine_version_tuple():
...
def register_devices(prefs, devices):
    ...
    for dev in devices:
        ...
        dev_desc = DeviceDesc(\
            dev_id,
            dev_name,
            supported=(dev_flags & DhpmDeviceFlags.SUPPORTED) > 0,
            supports_groups_together=(dev_flags & DhpmDeviceFlags.SUPPORTS_GROUPS_TOGETHER) > 0,
...
class DHPM_OT_SetEnginePath(bpy.types.Operator, ImportHelper):

    bl_idname = 'dpacker.set_engine_path'
    bl_label = 'Set Engine Path'
    bl_description = 'Set a path to the DHPM engine'
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({'INFO'}, 'DHPM engine initialized. Save Blender preferences to make the path permanent')

        except RuntimeError as ex:
            if in_debug_mode():
                print_backtrace(ex)

            self.report({'ERROR'}, 'DHPM engine initialization failed: ' + str(ex))

        except Exception as ex:
            if in_debug_mode():
                print_backtrace(ex)

            self.report({'ERROR'}, 'DHPM initialization failed: Unexpected error')
