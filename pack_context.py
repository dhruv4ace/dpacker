...
class PackIsland:
    ...
    def __init__(self, p_obj, face_indices, selected):
        self.p_obj = p_obj
        self.flags = DhpmIslandFlags.SELECTED if selected else 0
    ...
    def apply_out_island(self, out_island):
        ...
        if out_island.flags is not None:
            ...
            old_selected = (old_flags & DhpmIslandFlags.SELECTED) > 0
            new_selected = (new_flags & DhpmIslandFlags.SELECTED) > 0
...
class PackContext:
    ...
    def __init__(self, _context):
        ...
        self.iparam_array = [None] * DhpmIslandIntParams.MAX_COUNT
    ...
    def serialize_uv_maps(self, send_unselected, send_verts_3d, iparam_serializers):
        ...
        if send_unselected:
            serialization_flags |= DhpmMapSerializationFlags.CONTAINS_FLAGS
            face_flags_array = []

        if send_verts_3d:

            serialization_flags |= DhpmMapSerializationFlags.CONTAINS_VERTS_3D
            verts_3d_array = []
        ...
        for p_obj_idx, p_obj in enumerate(self.p_objects):
            ...
            for face in faces:
                ...
                if send_unselected:
                    ...
                    if face_selected:
                        face_flags |= DhpmFaceInputFlags.SELECTED
                        selected_cnt += 1
                    else:
                        unselected_cnt += 1
        ...
        if (serialization_flags & DhpmMapSerializationFlags.CONTAINS_VERTS_3D) > 0:
            serialized_maps += struct.pack('f' * len(verts_3d_array), *verts_3d_array)

        if (serialization_flags & DhpmMapSerializationFlags.CONTAINS_FLAGS) > 0:
        ...
        if in_debug_mode():
            print('DHPM serialization time: ' + str(time.time() - serialize_start))
    ...
    def set_islands(self, selected_count, islands):
        if selected_count > len(islands):
            raise RuntimeError('Unexpected output from the DHPM process')
